<template>
   <div @click="openSearch" class="search-tab" title="Поиск авторов"
      ...
      <form class="search-box" :class="{ 'pb-2': searchOpen, 'h-full': !searchOpen }" @submit.prevent>
         ...
         <input v-if="searchOpen" v-model="prompt" type="search" name="prompt" class="search-input color-auto"
            ref="inputRef" placeholder="авторы" @input="handleInput">
         <span v-else class="search-placeholder">авторы</span>
      </form>
      <ul class="hint-container scroll-hidden" :class="{ 'open': searchOpen }">
         <transition-group name="show">
            <li v-if="hints" class="color-auto hint btn" v-for="(a, idx) in hints" :key="idx" :class="{'border-2 border-(--color-base-300)': appStore.searchFilters.author?._id === a._id}">
               <div @click="authorToggle(a)">{{ a.name }}</div>
            </li>
         </transition-group>
      </ul>
   </div>
...
const hints = ref<import('@/types/models').Author[] | undefined>();
...
const searchHint = async () => {
   ...
   const res = await $api({
      base: 'query/author',
      query: { 'author-search': { prompt: prompt.value.trim(), limit: 6 }}
   });
   if(res && res.length > 0){
      hints.value = res
   }
...
const authorToggle = (author: import('@/types/models').Author) => {
   if(appStore.searchFilters.author){
      if(appStore.searchFilters.author._id === author._id){
         appStore.searchFilters.author = undefined
      }else{
         appStore.searchFilters.author = author
      }
   } else {
      appStore.searchFilters.author = author
   }
