      ...
      <div class="container my-2">
         <div class="container-vertical max-h-[30vh]">
            <template v-if="series">
               <router-link v-for="(serie, idx) in series" :to="{ name: 'collection', params: { id: serie._id } }"
                  :key="idx">
                  <div class="ui-border clickable series-badge color-auto">
                     <div>
   ...
   <div class="page" v-if="data">
      ...
      <section class="w-full grid grid-cols-3 gap-4">
         ...
         <div class="cover-container">
            ...
            <template v-if="series">
               ...
               <router-link v-else-if="series.length === 1" :to="{ name: 'collection', params: { id: series[0]._id } }">
                  <div class="ui-border clickable series-badge cover-series-badge color-auto" :class="{ absolute: data.cover }">
                     <div>
                        <p class="prop sm-title">Книга из цикла</p>
         ...
         <article class="col-span-2 flex flex-col gap-4">
            ...
            <!-- Description -->
            <p class="prop text-xl" v-if="data.description">{{ data.description }}</p>
            <!-- Tags -->
            <div class="container" v-if="data.tags">
               <div class="container-start">
                  <Tag v-for="item in data.tags" :key="item.count" :link="{ name: 'catalog', query: { tags: item } }">{{ item }}</Tag>
               </div>
            </div>
            <!-- Library | Rating -->
            <div class="grid grid-cols-2 gap-2">
               ...
               <div class="ui-border flex flex-col" @click="$router.push({ hash: '#comments' })">
                  ...
                  <div class="flex justify-between items-center h-full">
                     <h1 class="prop color-auto text-[0.9rem]">{{ data.rating }} <span
               ...
               <div class="ui-border grid grid-cols-1 row-auto gap-2 overflow-clip"
                  :class="{ 'big-list': Object.values(data.additional).length > 2 && !isBigListOpen }"
                  v-if="data.additional">
            ...
            <div class="col-span-2 w-full ui-border" v-if="appStore.isAuthorized && data">
               <div class="grid grid-cols-10 gap-4 w-full p-1">
                  <Bookmark :book-id="(route.params.id as string)" class="col-span-1" />
                  <BuyBtn
                     v-if="!appStore.userData.books.find((b: any) => b.bookid === route.params.id) && appStore.bookData && appStore.bookData.library._id !== '69dd2a351da95895fb2d9e06'"
                     :basecost="appStore.bookData.basecost" :title="appStore.bookData.title"
                     :prod-id="(route.params.id as string)" class="w-full text-2xl col-span-9" />
                  <button
                     v-else-if="appStore.bookData && appStore.bookData.library._id === '69dd2a351da95895fb2d9e06' && !appStore.userData.books.find((b: any) => b.bookid === route.params.id)"
                     class="btn btn-border title color w-full col-span-9"
                     :style="{ background: `var(${SUBSCRIBTION_STYLE[appStore.bookData.library._id as keyof typeof SUBSCRIBTION_STYLE].var})` }"
                     @click="purchaseBook($route.params.id as string)" title="Купить книгу">Добавить</button>
                  <button v-else class="btn btn-border title text-2xl w-full col-span-9"
                     @click="$router.push({ name: 'read', params: { id: $route.params.id } })" title="Читать книгу">Читать!</button>
               </div>
            </div>

            <!-- Comments -->
            <div class="grid grid-cols-2 gap-2" id="comments">
               <template v-if="data.comments && data.comments.length > 0">
                  <label class="btn btn-border text-center" for="spoiler-btn" title="Показать/скрыть спойлеры">
                     Спойлеры
                     <input type="checkbox" v-model="spoilers" id="spoiler-btn">
                  </label>
                  <button class="btn btn-border col-span-2 last:col-span-1" title="Сортировать по отрицательным">Сначала отрицательные</button>
               </template>

               <button @click="isNewComment = true" class="btn btn-border col-span-3" title="Написать комментарий"
            ...
            <div class="container">
               <div class="container-vertical h-[40vh] rounded-xl scroll-hidden"
                  v-if="data.comments && data.comments.length > 0">
                  <div class="comment"
                     v-for="(item, index) in (spoilers ? data.comments : data.comments.filter((b: any) => b.spoilers != true))"
                     :key="index">
                     <Icon :icon="COMMENT_ICON[item.score as keyof typeof COMMENT_ICON].icon" class="comment-icon"
                        :style="{ color: `var(${COMMENT_ICON[item.score as keyof typeof COMMENT_ICON].color})` }" />
                     <div class="comment-body color">
                        <p class="comment-text">{{ item.comment }}</p>
                        <div class="comment-bottom prop">
                           <p class="color font-semibold">{{ item.read ? 'Полностью прочитана' : 'Прочитана неполностью' }}</p>
                           <p class="color font-semibold"><span
                                 v-if="appStore.userData && item.user === appStore.userData._id">Ваш</span>
                              {{ parseDate(item.datestamp) }}</p>
                        </div>
                     </div>
                  </div>
               </div>
...
import { COMMENT_ICON, SUBSCRIBTION_STYLE } from '@/api/styles';
...
const saveNewComment = async () => {
   await $api({
      ...
      query: {
         ...
         read: appStore.userData.books.find((b: Record<string, undefined>) => b.bookid === data.value._id).lastpage > 80,
...
.comment {
   @apply flex gap-3 border-2 rounded-xl p-2;
   border-color: var(--color-base-100);

   .comment-icon {
      @apply rounded-lg p-1 size-10;
      background: var(--color-base-100);
   }

   .comment-body {
      @apply flex flex-col min-h-10 w-full justify-between gap-4;
